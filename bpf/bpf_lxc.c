...
skip_service_lookup:
	...
	case CT_NEW:
		 ...
		 * reverse NAT.
		 */
		ret = ct_create6(&CT_MAP6, tuple, skb, 0, rev_nat_index);
		if (IS_ERR(ret))
			return ret;

		/* Reset index to avoid attempting reverse translation later on */
		rev_nat_index = 0;
		break;
...
static inline int handle_ipv4(struct __sk_buff *skb)
{
	...
	if (!skip_service && (svc = lb4_lookup_service(skb, &key)) != NULL) {
		...
		ret = lb4_rev_nat(skb, l3_off, l4_off, &csum_off, rev_nat_index, &tuple, 0);
		if (IS_ERR(ret))
			return ret;
...
static inline int __inline__ ipv6_policy(struct __sk_buff *skb, int ifindex, __u32 src_label)
{
	...
	if (unlikely(src_rev_nat)) {
		ret = lb6_rev_nat(skb, l4_off, &off, src_rev_nat, &tuple, 0);
		if (IS_ERR(ret))
			return ret;
	}
...
static inline int __inline__ ipv4_policy(struct __sk_buff *skb, int ifindex, __u32 src_label)
{
	...
	if (unlikely(src_rev_nat)) {
		ret = lb4_rev_nat(skb, ETH_HLEN, l4_off, &off, src_rev_nat, &tuple,
